/**
 * Country dial codes used by the phone-input component.
 *
 * Compiled from the ITU-T E.164 numbering plan plus emoji flag glyphs.
 * Sorted alphabetically by country name. Default is India (matches the
 * primary launch market) but the picker lets users override.
 *
 * Each entry is a tuple of [iso2, name, dialCode, flag] kept compact to
 * minimise the gzipped payload — the full list is ~6 KB on the wire.
 */
...
export interface Country {
  ...
  dial: string;
  /** Emoji flag (rendered by the OS — no asset request) */
  flag: string;
}

/** ISO-2 of the default selection. Override per-form via PhoneInput prop. */
...
export const COUNTRIES: Country[] = [
  ...
  { iso2: "CX", name: "Christmas Island", dial: "61", flag: "🇨🇽" },
  { iso2: "CC", name: "Cocos (Keeling) Islands", dial: "61", flag: "🇨🇨" },
  ...
  { iso2: "CD", name: "Democratic Republic of the Congo", dial: "243", flag: "🇨🇩" },
  ...
  { iso2: "VI", name: "Virgin Islands (US)", dial: "1340", flag: "🇻🇮" },
  { iso2: "WF", name: "Wallis and Futuna", dial: "681", flag: "🇼🇫" },
  { iso2: "EH", name: "Western Sahara", dial: "212", flag: "🇪🇭" },
...
export const COUNTRIES_BY_ISO: Record<string, Country> = Object.fromEntries(
  COUNTRIES.map((c) => [c.iso2, c]),
);

export function findByIso(iso2: string): Country {
  return COUNTRIES_BY_ISO[iso2] ?? COUNTRIES_BY_ISO[DEFAULT_COUNTRY]!;
}
